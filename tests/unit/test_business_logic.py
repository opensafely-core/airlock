...
def test_group_comment_create_permissions_submitted_request(bll):
    author = factories.create_user("author", ["workspace"], False)
    collaborator = factories.create_user("collaborator", ["workspace"], False)
    other = factories.create_user("other", ["other"], False)
    checker = factories.create_user("checker", ["other"], True)
    ...
    # author cannot comment when request is in submitted status
    with pytest.raises(exceptions.RequestPermissionDenied):
        bll.group_comment_create(
            release_request, "group", "comment from author", Visibility.PUBLIC, author
        )

    # collaborator cannot comment (same as author)
    with pytest.raises(exceptions.RequestPermissionDenied):
        bll.group_comment_create(
    ...
    # output-checker with no access to workspace can comment
    bll.group_comment_create(
        release_request, "group", "checker", Visibility.PUBLIC, checker
    )
    ...
    # output-checker collaborator can comment
    bll.group_comment_create(
        release_request,
        "group",
        "collaborator_checker",
