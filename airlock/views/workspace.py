...
def multiselect_add_files(request, multiform, workspace):
    ...
    for f in multiform.cleaned_data["selected"]:
        ...
        if policies.can_add_file_to_request(workspace, relpath):
            if policies.filetype_is_allowed(workspace, relpath, RequestFileType.OUTPUT):
                files_to_add.append({"file": f})
            else:
                files_to_add.append({"file": f, "allow_output_filetype": False})
        else:
